...
for subject in $sublist; do
	...
	for i in ICVF ISOVF OD; do
		...
		antsApplyTransforms  \
		  -d 3 \
		  -r "${FMRIPREP_DIR}/${subject}_ses-01_space-MNI152NLin6Asym_desc-preproc_T1w.nii.gz" \
		  -t ${T1_MNI_TRANSFORM} -t [ ${T1_ACPC_TRANSFORM}, 1 ] \
		  -n NearestNeighbor \
		  -i "${AMICO_DIR}/${subject}_ses-01_space-T1w_desc-preproc_space-T1w_desc-${i}_NODDI.nii.gz" \
		  -o "${OUTPUT_DIR}/${subject}_ses-01_space-MNI152NLin6Asym_desc-${i}_NODDI.nii.gz" \
		  -v
